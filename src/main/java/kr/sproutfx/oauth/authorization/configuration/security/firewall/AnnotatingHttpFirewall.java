package kr.sproutfx.oauth.authorization.configuration.security.firewall;
    ...
    public FirewalledRequest getFirewalledRequest(final HttpServletRequest request)
    {
        try
        {
            ...
            if (requestUrl.contains(";jsessionid="))
            {
                ...
                {
                    LOGGER.log(Level.WARNING, String.format("Intercepted RequestBlockedException: Remote Host: %s, User Agent: %s, Request URL: %s", request.getRemoteHost(), request.getHeader("User-Agent"), request.getRequestURL().toString()));
                }
